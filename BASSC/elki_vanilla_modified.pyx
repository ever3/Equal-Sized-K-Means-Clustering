...
cdef assign(int[:] labels, double[:] mindist, int N, int k, np.ndarray all_distances_, np.ndarray sizes):
    cdef:
        double[:, :] all_distances = all_distances_
        #np.ndarray[long, ndim=1] cluster_space_ = np.full(k, max_cluster_size)
        long[:] cluster_space = sizes
...
cdef transfer_points(int[:] ilabels, double[:] mindist_, np.ndarray all_distances_, long[:] cluster_space, int N):
    ...
    for i in range(points_by_high_distance.shape[0]):
        ...
        # assign point to better cluster if possible
            ...
            if point_cluster != cand_cluster:
                ...
                if (point_distance_to_cand_cluster < cand_distance_to_cand_cluster):
                    ...
                    if mindist_sum <  best_mindist_sum:
                        ...
                        break
                        # wenn break rausgenommen wird, muss point_cluster geändert werden auf cand_cluster
...
def elki(X, k, centroids, sizes):
    ...
    cdef:
        long[:] cluster_space
    labels, mindist, cluster_space = assign(labels, mindist, N, k, all_distances, sizes)
